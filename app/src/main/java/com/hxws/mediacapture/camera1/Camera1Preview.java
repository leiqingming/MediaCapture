...
import com.hxws.mediacapture.camerabase.AspectRatio;
import com.hxws.mediacapture.camerabase.Constants;
import com.hxws.mediacapture.camerabase.Size;
import com.hxws.mediacapture.camerabase.SizeMap;
...
public class Camera1Preview extends SurfaceView implements CameraView, SurfaceHolder.Callback {
    ...
    private void initCamera() {
        ...
        Log.d(TAG,"initCamera: mCamera = "+mCamera);
        if (mCamera == null)
            return;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (0 == mRatioWidth || 0 == mRatioHeight) {
            ...
            //transformCamera(mRatioWidth,mRatioHeight);
            ...
            if (widthMeasure < heightMeasure * mRatioWidth / mRatioHeight) {
                ...
                setMeasuredDimension(heightMeasure * mRatioWidth / mRatioHeight, heightMeasure);
            }
        }
    }
...
//            Log.d(TAG, "transformCamera, mRatioWidth=" + mRatioWidth + "," + "mRatioHeight=" + mRatioHeight);
//            return;
//        }
//        float sx = (float) mRatioWidth / (float) cameraWidth;
//        float sy = (float) mRatioHeight / (float) cameraHeight;
...
//        matrix.preTranslate((mRatioWidth - cameraWidth) / 2, (mRatioHeight - cameraHeight) / 2);
//
//        //第1步:因为默认是fitXY的形式显示的,所以首先要缩放还原回来.
//        matrix.preScale(cameraWidth / (float) mRatioWidth, cameraHeight / (float) mRatioHeight);
//
//        //第3步,等比例放大或缩小,直到图像区的一边超过View一边, 另一边与View的另一边相等. 因为超过的部分超出了View的范围,所以是不会显示的,相当于裁剪了.
//        matrix.postScale(maxScale, maxScale, mRatioWidth / 2, mRatioHeight / 2);//后两个参数坐标是以整个View的坐标系以参考的
//
...
//    }
    ...
    private void setCameraParameters(int width, int height) {
        ...
        if (mAspectRatio == null) {
            mAspectRatio = Constants.FULL_ASPECT_RATIO;
        }
    ...
    private void savePictureToSDCard(byte[] data) {
        ...
        if (FileUtil.checkSDCard()) {
            if (mPictureSaveDir == null) {
                pictureFile = FileUtil.getOutputMediaFile(mContext, FileUtil.MEDIA_TYPE_IMAGE);
            } else {
                pictureFile = FileUtil.getTimeStampMediaFile(mPictureSaveDir, FileUtil.MEDIA_TYPE_IMAGE);
            }
            if (pictureFile == null) {
